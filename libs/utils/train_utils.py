...
def log_to_csv(experiment, log_file_path, epoch, mAP, best_checkpoint_path):
    # Check if log file exists, if not, create with headers
    file_exists = os.path.isfile(log_file_path)
    with open(log_file_path, 'a', newline='') as csvfile:
        fieldnames = ['id', 'epoch', 'mAP', 'best_checkpoint_path']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        if not file_exists:
            writer.writeheader()  # file doesn't exist yet, write a header

        writer.writerow({'id': experiment, 'epoch': epoch, 'mAP': mAP, 'best_checkpoint_path': best_checkpoint_path})
